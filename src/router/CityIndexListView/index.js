'use strict';
import React, { Component, } from 'react';
import { ListView,View, TouchableOpacity,Image,Text,Modal} from 'react-native';
import NavigatorTopBar from '../../components/common/NavigatorTopBar';
import color from '../../constant/color';
import Constants from '../../constant/constants'
import Dimensions from 'Dimensions';
let { width, height } = Dimensions.get('window');
import styles from './styles'
var _navigator,_state,_this;
export default class CityIndexListView extends Component {
    constructor(props) {
        super(props);
        var ds = new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2});
        this.state = {
          dataSource: ds.cloneWithRows(Constants.regionoptions),
          list_a_key:'',
          list_b_key:'',
          list_c_key:'',
          city:ds,
          visible:false,
          placeText:"请选择",
          cityText:"请选择",
          areaText:"请选择",
          placeShow:true,
          cityShow:false,
          areaShow:false,
          placeMenu:true,
          cityMenu:false,
          areaMenu:false,
          area:ds,
        };
     }
     componentDidMount(){
        console.log(Constants)
        console.log(Constants.regionoptions[10])
     }
     selectPlace(id,content){
       _this.setState({
         cityShow:true,
         placeShow:false,
         areaShow:false,
         cityMenu:true,
         placeText:content.label,
         list_b_key:'',
         list_a_key: id,
         city:new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2}).cloneWithRows(content.children),
         dataSource:new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2}).cloneWithRows(Constants.regionoptions),
       })
      //  if(content.children[_state.list_b_key]&&_state.areaShow){
      //    _this.setState({
      //      area:new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2}).cloneWithRows(content.children[_state.list_b_key].children),
      //    })
      //  }
       console.log(Constants.regionoptions[id])
     }

     selectTwo(id,content){
       _this.setState({
         areaShow:true,
         cityShow:false,
         placeShow:false,
         areaMenu:true,
         cityText:content.label,
         list_b_key: id,
         list_c_key:'',
         city:new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2}).cloneWithRows(Constants.regionoptions[_state.list_a_key].children),
         area:new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2}).cloneWithRows(content.children),
       })
     }
     selectThree(id,content){
       _this.setState({
         list_c_key: id,
         areaText:content.label,
         area:new ListView.DataSource({rowHasChanged: (r1, r2) =>r1!== r2}).cloneWithRows(Constants.regionoptions[_state.list_a_key].children[_state.list_b_key].children),
       })
     }
     _renderRowOne(rowContent,sectionID,rowID){
       return(
           <TouchableOpacity style={styles.labels} key={rowID} onPress={()=>_this.selectPlace(rowID,rowContent)}>
                <Text style={_state.list_a_key == rowID?styles.ltAtext:styles.lt_text}>{rowContent.label}</Text>
           </TouchableOpacity>
       )
     }
     _renderRowTwo(rowContent,sectionID,rowID){
       return(
           <TouchableOpacity style={styles.labels} key={rowID} onPress={()=>_this.selectTwo(rowID,rowContent)}>
                <Text style={_state.list_b_key == rowID?styles.ltAtext:styles.lt_text} numberOfLines={1}>{rowContent.label}</Text>
           </TouchableOpacity>
       )
     }
     _renderRowThree(rowContent,sectionID,rowID){
       return(
           <TouchableOpacity style={styles.labels} key={rowID} onPress={()=>_this.selectThree(rowID,rowContent)}>
                <Text style={_state.list_c_key == rowID?styles.ltAtext:styles.lt_text} numberOfLines={1}>{rowContent.label}</Text>
           </TouchableOpacity>
       )
     }
     changeStateOne(){
       _this.setState({
         areaShow:false,
         cityShow:false,
         placeShow:true,
         areaMenu:false,
         cityText:"请选择",
         areaText:"请选择",
       })
     }
     changeStateTwo(){
       _this.setState({
         areaShow:false,
         cityShow:true,
         placeShow:false,
         areaText:"请选择",
       })
     }
     changeStateThree(){
       _this.setState({
         areaShow:true,
         cityShow:false,
         placeShow:false,
         areaText:"请选择",
       })
     }

     render(){
       _this=this;
       _state=this.state;
       _navigator=this.props.navigation;
       return(
         <View style={{flex:1,position:"relative",}}>
         <Modal
           animationType="slide"
           transparent={true}
           visible={_state.visible}
           onRequestClose={() => {}}
           >
           <View style={{flex:1,height:300,position:"absolute",bottom:0,backgroundColor:color.back1C}}>
               <View style={{height:50,width:width,flexDirection: 'row',alignItems: 'center',justifyContent:"space-between",borderBottomWidth:0.5,borderColor:"#ccc"}}>
                   <View style={{flex:1}}>
                   </View>
                   <View style={{flex:1,alignItems:"center",}}>
                     <Text style={{fontSize:20,color:"#222"}} numberOfLines={1}>所在地区</Text>
                   </View>
                   <TouchableOpacity style={{flex:1,alignItems:"flex-end",}} onPress={()=>{}}>
                     <Text style={{fontSize:30,color:"#ccc",marginRight:12}} numberOfLines={1}>×</Text>
                   </TouchableOpacity>
               </View>

             <View style={{height:50,width:width,paddingLeft:12,flexDirection: 'row',borderBottomWidth:0.5,borderColor:"#ddd"}}>

                 <TouchableOpacity style={{width:100,justifyContent:"center",alignItems:"flex-start"}} onPress={()=>_this.changeStateOne()}>
                    <View style={_state.placeShow?{height:50,justifyContent:"center",borderColor:color.main1C,borderBottomWidth:2,}:{height:50,justifyContent:"center",}}>
                      <Text style={_state.placeShow?{fontSize:15,color:color.main1C}:styles.lt_text} numberOfLines={1}>{_state.placeText}</Text>
                    </View>
                </TouchableOpacity>
                {_state.cityMenu?
                  <TouchableOpacity style={{width:100,justifyContent:"center",alignItems:"flex-start"}} onPress={()=>_this.changeStateTwo()}>
                      <View style={_state.cityShow?{height:50,justifyContent:"center",borderColor:color.main1C,borderBottomWidth:2,}:{height:50,justifyContent:"center",}}>
                        <Text style={_state.cityShow?{fontSize:15,color:color.main1C}:styles.lt_text} numberOfLines={1}>{_state.cityText}</Text>
                      </View>
                   </TouchableOpacity>
                  :null}
                {
                  _state.areaMenu?<TouchableOpacity style={{width:100,justifyContent:"center",alignItems:"flex-start"}} onPress={()=>_this.changeStateThree()}>
                     <View style={_state.areaShow?{borderColor:color.main1C,borderBottomWidth:2,height:50,justifyContent:"center"}:{height:50,justifyContent:"center",}}>
                       <Text style={_state.areaShow?styles.ltAtext:styles.lt_text} numberOfLines={1}>{_state.areaText}</Text>
                      </View>
                   </TouchableOpacity>
                  :null
                }




             </View>

            <View style={{height:200,}}>
              <View style={styles.containerBox}>
              {
                _state.placeShow?
                <ListView
                 showsVerticalScrollIndicator ={false}
                  dataSource={this.state.dataSource}
                  renderRow={this._renderRowOne}
                  initialListSize={10}
                  contentContainerStyle={styles.container}
                />
                :
                null
              }

               {
                 _state.cityShow?
                   <ListView
                   style={{flex:3}}
                   iosalwaysBounceVertical={false}
                   showsVerticalScrollIndicator ={false}
                     dataSource={_state.city}
                     renderRow={this._renderRowTwo}
                     initialListSize={10}
                     contentContainerStyle={styles.container}
                   />
                 :null
               }
               {
                 _state.areaShow?

                   <ListView
                    style={{flex:2}}
                    showsVerticalScrollIndicator ={false}
                     dataSource={_state.area}
                     renderRow={this._renderRowThree}
                     initialListSize={10}
                     contentContainerStyle={styles.container}
                   />

                 :null
               }

                      </View>
                    </View>
                 </View>
          </Modal>
          </View>
       )
     }
   }
