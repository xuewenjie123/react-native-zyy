...
import { View, Image, ScrollView,RefreshControl, DeviceEventEmitter,TouchableOpacity,Text, ToastAndroid,ListView ,BackHandler} from 'react-native';//InteractionManager
...
export default class ConsultationMy extends Component {
  ...
  componentDidMount(){
   ...
   _this.subscription = DeviceEventEmitter.addListener("changeMyUI",function(){
      _this.setState({
        reset:true
      })
      getStorage("login",function(error,data){
        if(data&&data.userType=="expert"){
            getQuestion({page:1,expert:""},_this.getQuestionResult)
        }else{
    ...
    return(
      <View style={styles.dat1} key={rowID}>
        <View style={styles.dat2}>
          <Image style={{width: 25, height: 25,marginRight:10,borderRadius:12.5}} source={rowData.url?{uri:rowData.url}:require('../../../images/head_portrait.png')}></Image>
        ...
        <View style={styles.dat3}>
          ...
          <View style={styles.dat4}>
            ...
            <View style={styles.dat5}>
              ...
              <TouchableOpacity onPress={()=>_navigator.navigate("ConsultationDetail",{questionId:rowData.id_,My:true})}>
                <Text style={styles.cont_text5}>
                  {"查看详情 >> "}
                </Text>
              </TouchableOpacity>
  ...
  onEndReached(){
    console.log(_state.page*_state.size<_state.total);
          ...
          {_state.loadingList?
            _state.getList.length?
              ...
              :<View style={styles.noneBox}>
                  ...
                  <Text style={{fontSize:15,color:color.font2C,lineHeight:15,}}>您还没有发起过问题</Text>
                </View>
          }
